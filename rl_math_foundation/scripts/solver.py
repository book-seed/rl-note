...
class Solve:
            ...
            for state in range(self.state_space_size):
                ...
                for action in range(self.action_space_size):
                    value += policy[state, action] * self.calculate_qvalue(
                        state_value=state_value_k.copy(), state=state, action=action
                    )  # bootstrapping
    ...
    def policy_improvement(self, state_value):
        ...
        for state in range(self.state_space_size):
            ...
            for action in range(self.action_space_size):
                qvalue_list.append(
                    self.calculate_qvalue(state, action, state_value.copy())
                )
    ...
    def policy_iteration(self, tolerance=0.001, steps=100):
        ...
        while np.linalg.norm(policy - self.policy, ord=1) > tolerance and steps > 0:
            ...
            self.state_value = self.policy_evaluation(
                self.policy.copy(), tolerance, steps
            )
    ...
    def obtain_episode(self, policy, start_state, start_action, length):
        ...
        while length > 0:
            ...
            next_action = np.random.choice(
                np.arange(len(policy[next_state])), p=policy[next_state]
            )
            episode.append(
                {
                    "state": state,
    ...
    def mc_epsilon_greedy(self, length=1000, epsilon=1, tolerance=1):
        ...
        while True:
            ...
            if (
                len(norm_list) >= 3
                and norm_list[-1] < tolerance
                and norm_list[-2] < tolerance
                and norm_list[-3] < tolerance
            ):
                break
            ...
            for step in range(len(episode) - 1, -1, -1):
                ...
                for a in range(self.action_space_size):
                    if a == action_star:
                        self.policy[state, a] = (
                            1
                            - ((self.action_space_size - 1) / self.action_space_size)
                            * epsilon
                        )
                    else:
        ...
        while num_episodes > 0:
            ...
            while not done:
                ...
                action = np.random.choice(
                    np.arange(self.action_space_size), p=self.policy[state]
                )
                _, reward, done, _, _ = self.env.step(action)
                # print("done:", done)
                episode_length += 1
                ...
                next_state = self.env.pos2state(self.env.agent_location)
                next_action = np.random.choice(
                    np.arange(self.action_space_size), p=self.policy[next_state]
                )
    ...
    def expected_sarsa(self, alpha=0.1, epsilon=1, num_episodes=1000):
        ...
        while num_episodes > 0:
            ...
            num_episodes -= 1
            print(
                np.linalg.norm(qvalue_list[-1] - qvalue_list[-2], ord=1), num_episodes
            )
            while not done:
                state = next_state
                action = np.random.choice(
                ...
                for next_action in range(self.action_space_size):
                    expected_qvalue += (
                        self.qvalue[next_state, next_action]
                        * self.policy[next_state, next_action]
                    )
        ...
        self.env.render_.add_subplot_to_fig(
            fig=fig,
            x=episode_index_list,
            y=reward_list,
            subplot_position=211,
            xlabel="episode_index",
            ylabel="total_reward",
        )
        self.env.render_.add_subplot_to_fig(
            fig=fig,
    ...
    def q_learning_off_policy(
        self, alpha=0.01, epsilon=0.1, num_episodes=2000, episode_length=2000
    ):
        start_state = self.env.pos2state(self.env.agent_location)
        start_action = np.random.choice(
            np.arange(self.action_space_size), p=self.mean_policy[start_state]
        )
        episode = self.obtain_episode(
            self.mean_policy.copy(),
            ...
            for i in range(ord + 1):
                for j in range(ord + 1):
                    feature_vector.append(
                        np.cos(np.pi * (i * x_normalized + j * y_normalized))
                    )
    ...
    def gfv_a(self, fourier: bool, state: int, action: int, ord: int) -> np.ndarray:
        ...
        if (
            state < 0
            or state >= self.state_space_size
            or action < 0
            or action >= self.action_space_size
        ):
        ...
        if fourier:
            ...
            # 归一化到 0 到 1
            state_normalized = (state - (self.state_space_size - 1) * 0.5) / (
                self.state_space_size - 1
            )
            action_normalized = (action - (self.action_space_size - 1) * 0.5) / (
                self.action_space_size - 1
            )
            for i in range(ord + 1):
                for j in range(i + 1):
                    feature_vector.append(
                        state_normalized ** (ord - i) * action_normalized**j
                    )
        return np.array(feature_vector)

    def td_value_approximation(
        self, learning_rate=0.0005, epochs=500000, fourier=False, ord=1
    ):
        self.state_value = self.policy_evaluation(self.policy)
        if (
            not isinstance(learning_rate, float)
            or not isinstance(epochs, int)
            or not isinstance(fourier, bool)
            or not isinstance(ord, int)
        ):
        ...
        episode = self.obtain_episode(
            self.mean_policy, start_state, start_action, length=episode_length
        )
    ...
    def sarsa_function_approximation(
        self,
        ...
        for episode in range(num_episodes):
            ...
            while not done:
                state = next_state
                action = np.random.choice(
                    np.arange(self.action_space_size), p=policy[state]
                )
                _, reward, done, _, _ = self.env.step(action)
                episode_length += 1
                total_rewards += reward
                next_state = self.env.pos2state(self.env.agent_location)
                next_action = np.random.choice(
                    np.arange(self.action_space_size), p=policy[next_state]
                )
                target = reward + self.gama * np.dot(
                    self.gfv_a(fourier, next_state, next_action, ord), w
                )
                ...
                # for state in range(self.state_space_size):
                #     for action in range(self.action_space_size):
                qvalue_approximation[state, action] = np.dot(
                    self.gfv_a(fourier, state, action, ord), w
                )
            ...
            reward_list.append(total_rewards)
            length_list.append(episode_length)
            print(f"episode={episode},length={episode_length},reward={total_rewards}")
        ...
        ax_rmse.set_ylabel("RMSE")
        X, Y = np.meshgrid(
            np.arange(0, self.action_space_size), np.arange(0, self.state_space_size)
        )
        Z = self.qvalue
    ...
    def qlearning_function_approximation(
        self,
        learning_rate=0.0005,
        epsilon=0.1,
        num_episodes=100000,
        ...
        for episode in range(num_episodes):
            ...
            while not done:
                ...
                for s in range(self.state_space_size):
                    for a in range(self.action_space_size):
                        qvalue_approximation[s, a] = np.dot(
                            self.gfv_a(fourier, s, a, ord), w
                        )
                qvalue_star = qvalue_approximation[state].max()
                action_star = qvalue_approximation[state].tolist().index(qvalue_star)
                for a in range(self.action_space_size):
            ...
            self.writer.add_scalar(
                "rmse",
                np.sqrt(np.mean((qvalue_approximation - self.qvalue) ** 2)),
                episode,
            )
    ...
    def qvalue_function_approximation(
        self,
        ...
        for epoch in range(num_episodes):
            ...
            w = w + learning_rate * error * gradient
            for a in range(self.action_space_size):
                qvalue_approximation[state, a] = np.dot(
                    self.gfv_a(fourier, state, a, ord), w
                )
            self.writer.add_scalar(
                "rmse",
                np.sqrt(np.mean((qvalue_approximation - self.qvalue) ** 2)),
            ...
            if epoch % 1000 == 0:
                print(
                    epoch, np.sqrt(np.mean((qvalue_approximation - self.qvalue) ** 2))
                )
        X, Y = np.meshgrid(
            np.arange(0, self.action_space_size), np.arange(0, self.state_space_size)
        )
        ...
        for i in range(self.state_space_size):
            for j in range(self.action_space_size):
                print(
                    f"qvalue:{self.qvalue[i, j]},approximation:{qvalue_approximation[i, j]}"
                )
    ...
    def dqn(
        ...
        approximation_q_value = np.zeros(
            shape=(self.state_space_size, self.action_space_size)
        )
        i = 0
        rmse_list = []
        ...
        for epoch in range(epochs):
            for state_action, reward, next_state in date_iter:
                ...
                for action in range(self.action_space_size):
                    s_a = torch.cat(
                        (next_state, torch.full((batch_size, 1), action)), dim=1
                    )
                    q_value_target = torch.cat(
                        (q_value_target, q_target_net(s_a)), dim=1
                    )
                ...
                if i % update_step == 0 and i != 0:
                    q_target_net.load_state_dict(q_net.state_dict())  # 更新目标网络
                    # policy = np.zeros(shape=(self.state_space_size, self.action_space_size))
            ...
            for s in range(self.state_space_size):
                ...
                for a in range(self.action_space_size):
                    approximation_q_value[s, a] = float(
                        q_net(torch.tensor((y, x, a)).reshape(-1, 3))
                    )
    ...
    def obtain_episode_p(self, policy_net, start_state, start_action):
        ...
        while not done:
            ...
            next_action = np.random.choice(
                np.arange(self.action_space_size), p=prb.detach().numpy()
            )
            episode.append(
                {
                    "state": state,
