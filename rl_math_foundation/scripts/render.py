...
class Render:
    ...
    def _ensure_axes(self) -> None:
        ...
        self.ax.grid(
            True, which="major", linestyle="-", color="gray", linewidth=1, axis="both"
        )
        ...
        for lbl in self.ax.get_xticklabels(minor=True) + self.ax.get_yticklabels(
            minor=True
        ):
    ...
    def draw_random_line(
        self, pos1: list | tuple | np.ndarray, pos2: list | tuple | np.ndarray
    ) -> None:
        """
    ...
    def upgrade_agent(
        ...
        pos: list | np.ndarray | tuple,
        action,
        next_pos: list | np.ndarray | tuple,
    ) -> None:
        """
        更新agent的位置
    ...
    def show_frame(self, t: float = 0.2, block: bool = False) -> None:
        ...
        except Exception as e:  # noqa: BLE001 (best-effort refresh across backends)
            logger.warning("Error flushing events: %s", e)
...
if __name__ == "__main__":
    render = Render(
        target=[4, 4], forbidden=[np.array([1, 2]), np.array([2, 2])], size=5
    )
