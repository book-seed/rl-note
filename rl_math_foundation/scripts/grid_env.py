...
class GridEnv(gym.Env):
    def __init__(
        ...
        forbidden: list | tuple | np.ndarray,
        render_mode: str,
        ...
        # 初始化 动作空间 观测空间
        # seed = 42, "42 是生命、宇宙和一切终极问题的答案"
        self.action_space = spaces.Discrete(5, seed=42)
        self.action_space_size = self.action_space.n  # 从0开始索引
        print(
            f"self.action_space:{self.action_space}, self.action_space_size:{self.action_space_size}"
        )
        ...
        if len(reward_list) != 4:
            raise ValueError(
                "reward_list must have length 4: [other, target, forbidden, overflow]"
            )
    ...
    def step(
        self, action: ActType
    ) -> tuple[ObsType, float, bool, bool, dict]:  #  -> 是函数的返回类型注解，

        reward = self.reward_list[
            self.Rsa[self.pos2state(self.agent_location), action].tolist().index(1)
        ]
        direction = self.action_to_direction[action]
        self.render_.upgrade_agent(
            self.agent_location, direction, self.agent_location + direction
        )
    ...
    def render(
        self, t: float = 0.3, block: bool = False
    ...
    def state2pos(self, state: int) -> np.ndarray:
        ...
        比如在一个二维的格子世界中，我们可能会将每个格子看作一个状态，然后用一个整数来表示这个状态。
        而这个函数就是用来将这个整数转换回对应的格子位置。
    ...
    def psa_rsa_init(self):
        ...
        self.Psa = np.zeros(
            shape=(state_size, self.action_space_size, state_size), dtype=float
        )
        self.Rsa = np.zeros(
            shape=(state_size, self.action_space_size, len(self.reward_list)),
            dtype=float,
        )
        # 填充Psa、Rsa矩阵
        for state_index in range(state_size):
            for action_index in range(self.action_space_size):
                ...
                next_pos = (
                    pos + self.action_to_direction[action_index]
                )  # action_index：0~4，将动作映射给pos，得到next_pos

                if (
                    next_pos[0] < 0
                    or next_pos[1] < 0
                    or next_pos[0] > self.size - 1
                    or next_pos[1] > self.size - 1
                ):  # 如果“撞墙”了，超出地图边界
